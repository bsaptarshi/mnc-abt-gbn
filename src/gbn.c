...
int B_expected_seqnum = 0;	// to store what sequence number receiver is expecting
int A_next_seqnum = 0;	//to store what message sender must transmit next
int A_base = 0;	//base / start of window for sender
int A_buff = 0;	// receiver's buffer pointer
struct pkt A_buffer[1000];	// sender's message buffer
struct pkt B_pkt_copy;	//copy of received message at B
struct pkt ack_copy;	// copy of last sent out ack, for re-transmission purposes
...
struct msg message;
{
	...
	msg[20] = '\0';
	memcpy(msg, message.data, 20);
	A_application++;
	if (A_next_seqnum < (A_base + WINSIZE)) {		// if message falls within window, tarnsmit it
		int chk_sum = generate_checksum(A_next_seqnum, A_next_seqnum, msg);
		struct pkt temp;
		temp.seqnum = A_next_seqnum;
		temp.acknum = A_next_seqnum;
		temp.checksum = chk_sum;
		strncpy(temp.payload, msg, (strlen(msg)+1));	// generate packet and transmit to layer 3
		A_buffer[A_buff] = temp;
		tolayer3(0, A_buffer[A_next_seqnum]);
		A_transport++;
		if (A_base == A_next_seqnum) {	// if correct messsage is delivered, start the timer
			starttimer(0, TIMEOUT);
		}
		A_next_seqnum++;	//increment both buffer pointer and the expected next sequence number
		A_buff++;
	} else {	//if message outside window, buffer it to be sent later
		int chk_sum = generate_checksum(A_buff, A_buff, msg);
		struct pkt temp1;
		temp1.seqnum = A_buff;
		temp1.acknum = A_buff;
		temp1.checksum = chk_sum;
		strncpy(temp1.payload, msg, (strlen(msg)+1));
		A_buffer[A_buff++] = temp1;
		return;
	}
}
...
struct pkt packet;
{
	...
	int chk_sum = generate_checksum(packet.seqnum, packet.acknum, packet.payload);	// generate checksum to compare message corruption
	if (chk_sum == packet.checksum) {
		A_base = packet.acknum + 1;
		if (A_base == A_next_seqnum) {		// if the expected packets ACK is received, stop the timer, else restart the timer waiting for expected ACK
			stoptimer(0);
		} else {
			stoptimer(0);
			starttimer(0, TIMEOUT);
...
{
	starttimer(0, TIMEOUT);					// resend all packets in window starting from the oldest unacknowledged packet and start timer
	int i = A_base;
	while(i < (A_next_seqnum - 1)) {
...
struct pkt packet;
{
	...
	if (B_expected_seqnum != packet.seqnum) {	//if either pakett or ack was lost, this will result in outoforder packet
		if (B_expected_seqnum < packet.seqnum) {	// packet with smaller sequence number was lost
			printf("Packet %d not yet delivered. Dropping packet %d!!", B_expected_seqnum, packet.seqnum);
		} else {	//last sent ack was lost
			printf("ACK %d not yet delivered. Dropping packet %d!!", B_expected_seqnum, packet.seqnum);
		}
		tolayer3(1, ack_copy);	//resend ack of last acknowledged message
		return;
	} else {
		int chk_sum = generate_checksum(packet.seqnum, packet.acknum, packet.payload);
		if (chk_sum != packet.checksum) {	//check if packet contents are corupt or not, if yes, drop paccket
			printf("\n Corrupt message received at B....Dropping packet %s", packet.payload);
			fflush(stdin);
			return;
		} else {
			struct pkt ack;
...
{
	tolayer3(1, ack_copy);	// send a copy of the last acknowledged packet
}
